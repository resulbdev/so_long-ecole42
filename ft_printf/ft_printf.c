...
/*   Updated: 2023/01/18 17:40:25 by rbozdemi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	s_control(char *str)
{
	...
	if (str == NULL)
	{
		ft_putstr_fd2("(null)", 1);
		return (6);
	}
	else
		i = ft_putstr_fd2(str, 1);
	return (i);
}
...
static int	ft_check(va_list arg, const char c)
{
	...
	i = 0;
	if (c == 'c')
		i += ft_putchar_fd2(va_arg(arg, int), 1);
	else if (c == 's')
	...
	else if (c == 'p')
	{
		...
		i += ft_putnbr_fd2(va_arg(arg, int), 1);
	else if (c == 'u')
